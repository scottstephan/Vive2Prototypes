...
function Start () {
    ...
    /* TODO: This can be used to set the default directory.
       Unfortunately, it's not clear what to set it to (can't easily determin home directory from unity...)
    if (objectToSendTo == null) {
        objectToSendTo = gameObject;
    }
    objectToSendTo.SendMessage ("SetPath", "~");
    */
	...
	fileWindowRect.x = ( CameraManager.ScreenWidth / 2 ) + 5;
...
function OnGUI () {
	...
	if (!showMessageWindow) {
		GUI.BringWindowToFront (fileWindowID);
...
private function GetCurrentFileInfo () {
	...
	if (!limitToInitialFolder && dirInfo.Length > 0) {
		for (var i = 0; i < dirInfo.Length; i++) {
			...
			try {
				var _testDir = dirInfo[i].GetDirectories();
			}
			catch(err) {
				continue;
			}
...
public function CloseFileWindow ( cancel_window : boolean ) {
	...
	fileName = "";
	if (cancel_window)
		SendMessage ("FileWindowCanceled", SendMessageOptions.DontRequireReceiver);
	else
