...
public class NoiseEffect : MonoBehaviour
{
	...
	protected void Start ()
	{
		...
		{
//			log.Warning( "Noise shaders are not set up! Disabling noise effect." );
			enabled = false;
		}
		else
		{
	...
	void OnRenderImage (RenderTexture source, RenderTexture destination)
	{
		...
		mat.SetVector("_GrainOffsetScale", new Vector4(
			Random.value,
			Random.value,
			(float)CameraManager.ScreenWidth / (float)grainTexture.width * grainScale,
			(float)CameraManager.ScreenHeight / (float)grainTexture.height * grainScale
		));
		mat.SetVector("_ScratchOffsetScale", new Vector4(
			scratchX + Random.value*scratchJitter,
			scratchY + Random.value*scratchJitter,
			(float)CameraManager.ScreenWidth / (float) scratchTexture.width,
			(float)CameraManager.ScreenHeight / (float) scratchTexture.height
		));
