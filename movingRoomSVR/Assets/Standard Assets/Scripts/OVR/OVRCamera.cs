...
//WEMOLAB BEGIN
    private float playerScale = 1f; // use to make player feel smaller or larger
...
//WEMOLAB BEGIN
		GetComponent<Camera>().transform.localPosition = NeckPosition * playerScale;
//WEMOLAB END	
		...
		foreach(OVRCameraGameObject obj in CameraLocalSetList)
		{
			if(obj.CameraController.GetCameraDepth() == GetComponent<Camera>().depth)
			{
				// Initial difference
//WEMOLAB BEGIN
                Vector3 newPos = -(qp * CameraPositionOffset * playerScale);
//WEMOLAB END
                // Final position
...
//WEMOLAB BEGIN
		GetComponent<Camera>().transform.localPosition += Quaternion.Inverse(GetComponent<Camera>().transform.parent.rotation) * qp * newCamPos * playerScale;
//WEMOLAB END
		Vector3 newEyePos = Vector3.zero;
		newEyePos.x = EyePosition.x;
//WEMOLAB BEGIN
		GetComponent<Camera>().transform.localPosition += GetComponent<Camera>().transform.localRotation * newEyePos * playerScale;
//WEMOLAB END
}
