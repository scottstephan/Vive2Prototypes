...
public class OVRDevice : MonoBehaviour 
{
	...
	void Update()
	{
		if (HMD != null && Input.anyKeyDown && HMD.GetHSWDisplayState().Displayed){
			...
			//WEMOLAB BEGIN
			if(OculusFPSCameraMode.singleton != null)
				OculusFPSCameraMode.singleton.WarningTitleDismissed();
			//WEMOLAB END
		}

        //WEMOLAB BEGIN
        // subsequent runs may not show the warning, so we need to let the game know to proceed
        if (HMD != null && !HMD.GetHSWDisplayState().Displayed)
        {
            if(OculusFPSCameraMode.singleton != null && !OculusFPSCameraMode.singleton.WasWarningTitleDismissed())
            {
                OculusFPSCameraMode.singleton.WarningTitleDismissed();
            }
        }
