...
//WEMOLAB BEGIN
	...
	public Color OverlayColorOverride = new Color(0f, 0f, 0f, 0f);
	static OVRMainMenu singleton;
//WEMOLAB END
...
//WEMOLAB BEGIN
				...
				OculusScaler oculusScaler = CameraController.GetComponent<OculusScaler>();
				if (oculusScaler != null)
				{
					playerScale = oculusScaler.Scale;
				}
//WEMOLAB END

                // Grab transform of GUI object
//WEMOLAB BEGIN
                Vector3 ls = playerScale * GUIRenderObject.transform.localScale;
//WEMOLAB END
...
//WEMOLAB BEGIN
                GUIRenderObject.transform.localPosition = playerScale * lp;
//WEMOLAB END
...
//WEMOLAB BEGIN
	// similar to OnGUI, sets up render texture and transform, but only renders the overlay
	void WEMODrawOverlay(Color c)
	{
...
//WEMOLAB BEGIN
        if( startDelayTimer > 0f ) {
			...
			WEMODrawOverlay(new Color(0,0,0,AlphaFadeValue));
			return;
        }
//WEMOLAB END
		// Fade in screen
...
//WEMOLAB BEGIN
        ...
        if (OverlayColorOverride.a > 0)
        {
			WEMODrawOverlay(OverlayColorOverride);
			return;
		}
//WEMOLAB END
...
//WEMOLAB BEGIN
	static public void Reset()
	{
        if (singleton != null) 
        {
            singleton.AlphaFadeValue = 1f;
			singleton.startDelayTimer = singleton.startDelayTime;
        }
	}

    public static void SetOverlayColor(Color c)
    {
        if (singleton != null)
        {
            singleton.OverlayColorOverride = c;
        }
    }

	public static bool IsWarningActive()
	{
		//return (singleton != null && singleton.VisionGuide.GetFadeAlphaValue() > 0.3f); // seems to have a low amount of alpha sometimes, weird.
		return (singleton != null && singleton.VisionGuide.GetFadeAlphaValue() > 0.03f && OVRDevice.HMD.GetHSWDisplayState().Displayed ); 
	}
//WEMOLAB END
}
