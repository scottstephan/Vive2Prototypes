...
public class SingletonBehaviour<T> : MonoBehaviour where T : Component
{
        ...
        get
        {
            if (instance == null)
            {
                ...
                if (instance == null)
                {
                    ...
                    {
                        Log.Main.Warning("Attempt to access " + typeof(T).GetType().Name + " singleton before Start or without singleton being attached.");
                        hasWarned = true;
                    }
                }
            }
    ...
    protected static T CreateSingletonInstance()
    {
        ...
        Log.Main.Warning("Could not find " + gameObject.name + ", creating");
