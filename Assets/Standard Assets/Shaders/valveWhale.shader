Shader "Valve/valveWhale" {
  ...
  SubShader {
    ...
    void myvert (inout appdata_full v, out Input data) {
		...
		float4 wn = mul(_Object2World,half4(v.normal.x,v.normal.y,v.normal.z,1));
		data.causUV.w = clamp((dot(normalize(v.normal),half3(0,1,0)) + 0.5) / 1.5,0,1);
    ...
    void surf (Input IN, inout SurfaceOutput o) {
		...
		half texcaustic = tex2D (_CausticTex, IN.causUV.xy).g * IN.causUV.w;
